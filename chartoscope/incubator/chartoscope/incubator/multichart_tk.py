...
class RenkoChart:
    def __init__(self, root, dataset, grid_dimension, grid_location):
        self._root = root
        ...
        canvas1.get_tk_widget().grid(row=9, column=1)

        toolbar_frame = tk.Frame(self._root)
        toolbar_frame.config(bg='black')
        toolbar_frame.grid(row=10, column=1)
...
class ChartArea:
    def __init__(self, root, main_plot, grid_dimension, grid_location):
        self._chart = None
        if main_plot.plot_info == PlotType.Stock:
            ...
            self._chart = RenkoChart(root, main_plot, grid_dimension, grid_location)
...
class Multichart:
    ...
    def add_chart_area(self, root, main_plot, grid_location=None):
        chart_area = ChartArea(root, main_plot, self._grid_dimension, grid_location)
