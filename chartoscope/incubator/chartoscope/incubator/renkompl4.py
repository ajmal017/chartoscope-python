...
class RenkoFileUtility:
    ...
    def load_dataset_from_file(renko_file):
        ...
        renkos = []
        while x <= y:
            append_line = dates[x], openp[x], closep[x], hma_value[x], hma_roc[x], rsi_value[x]
...
def PlotRenko(dataset):
    ...
    rsi_indicator = list(map(lambda item: item[4], dataset.dataset))
    ...
    for item in dataset.dataset:
        ...
        if hma_roc > 90:
            if entry_position == MarketPosition.Short:
            ...
            elif entry_position == MarketPosition.NoPosition:
                if target_reversal is None or (target_reversal is not None) and target_reversal == MarketPosition.Long:
            ...
            if entry_position == MarketPosition.Long:
                ...
                if target_reversal is None or (target_reversal is not None and target_reversal == MarketPosition.Short):
                    entry_price = close[index]
            ...
            if entry_position == MarketPosition.Short and (hma_roc < 90 or hma_indicator1[index] < close[index]):
                pnl = (entry_price - close[index]) * 10000
                print('Closing sell @P/L {0}'.format(pnl))
                total_pnl += pnl
                entry_position = MarketPosition.NoPosition
                target_reversal = MarketPosition.Long
            elif entry_position == MarketPosition.Long and (hma_roc > 90 or hma_indicator1[index] > close[index]):
                pnl = (close[index] - entry_price) * 10000
                print('Closing buy @P/L {0}'.format(pnl))
                total_pnl += pnl
                entry_position = MarketPosition.NoPosition
